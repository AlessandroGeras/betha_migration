        ...
        try {
            ...
            for (let remessa of remessas) {
                if (remessa.status !== 'Concluído' && remessa.status !== 'Validado' && remessa.query && remessa.api) {
                    ...
                    const teste = queryResult.recordset.map(obj => JSON.stringify(obj)).join('\n');
                    console.log(teste);
                    ...
                    try {
                        ...
                        const response = await fetch(remessa.api, {
                            ...
                            body: queryResult.recordset.map(obj => JSON.stringify(obj)).join('\n') // Ajuste aqui
                        });
            ...
            for (let banco of bancos) {
                ...
                if (scriptsStatus.length === 0) {
                    console.log
                    (`Nenhum script encontrado para o banco: ${banco.banco}`);
                    continue;
                }
...
async function updateBankStatus(connection, id, status) {
    ...
    const request = new sql.Request(connection);
    request.input('status', sql.NVarChar, status);
    request.input('id', sql.Int, id);  // Correção: Mudança de 'status' para 'id'
    await request.query(updateQuery);
}
