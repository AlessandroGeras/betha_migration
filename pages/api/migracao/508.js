...
`;
        ...
        const transformedData = resultData.map(record => {
            ...
            // Parse JSON string for relacaoTiposServicoCreditosTributarios
            const relacaoTiposServicoCreditosTributarios = JSON.parse(record.relacaoTiposServicoCreditosTributarios);
        
            return {
                idIntegracao: record.idIntegracao.toString(),
                relacaoTiposServicoCreditosTributarios: {
                    idAgrupamentos: relacaoTiposServicoCreditosTributarios.idAgrupamentos,
                    idCreditoTributario: relacaoTiposServicoCreditosTributarios.idCreditoTributario
                }
            };
        });
        ...
        for (const record of transformedData) {
            ...
            if (idIntegracao) {
                try {
                    ...
                    const response = await fetch(`https://tributos.betha.cloud/service-layer-tributos/api/relacaoTiposServicoCreditosTributarios`, {
                        method: 'POST',
                        headers: {
