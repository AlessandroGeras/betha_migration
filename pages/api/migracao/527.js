...
WHEN CAST(pr.ds_produto AS VARCHAR(MAX)) = 'AÇÚCAR CRISTAL, PACOTE DE 02KG  ' then 27193116
 ...
 WHEN CAST(pr.ds_produto AS VARCHAR(MAX)) = 'PILHA  AAA  ALCALINA C/02 UNIDADES' then 27193240
	...
	WHERE sg_direcao = 'CD' and aa_movimento = '2024' and qt_movimento < 0 and fl_devolucao is null and  (cd_almoxa = 20100)
        `;
        ...
        const transformedData = resultData.map(record => {
            return {
                conteudo: {
                    ...
                    valorUnitario: Math.abs(record.valorUnitario), // Valor unitário do item
                    precoMedio: Math.abs(record.precoMedio), // Preço médio, se disponível
