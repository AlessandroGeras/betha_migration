...
CONCAT('DEDUÇÃO MIGRACAO DA RECEITA: ', r.ds_receita)        as finalidade,
JSON_QUERY(
(SELECT
ct.dt_movto       as data,
JSON_QUERY(
...
and r.cd_receitaPrevisaoTipo in (95,99)
        ...
        const transformedData = resultData.map(record => {
            ...
            return {
                ...
                content: {
                    ...
                    deducao: {
                        id: content.deducao?.id // Usando o id da dedução
                    },
                    credor: {
                        id: content.credor?.id // Usando o id do credor
                    },
                    finalidade: content.finalidade, // Usando a finalidade retornada
                    baixa: {
                        data: formatDate(content.baixa?.data), // Formatando a data da baixa
                        transacaoFinanceira: {
                            id: content.baixa?.transacaoFinanceira?.id // Usando o id da transação financeira
                        },
                        contaBancariaCredor: {
                            id: content.baixa?.contaBancariaCredor?.id // Usando o id da conta bancária do credor
                        },
                        numeroDocumento: content.baixa?.numeroDocumento?.toString(), // Convertendo o número do documento para string
                        contaBancariaEntidade: {
                            id: content.baixa?.contaBancariaEntidade?.id // Usando o id da conta bancária da entidade
                        }
                    },
                    receitas: [{
                        receita: {
                        ...
                        recursos: [{
                            ...
                            recurso: {
                                id: content.receitas?.recursos?.recurso?.id // Usando o id do recurso
                            }
                        }]
                    }]
                }
        ...
        /* for (const record of transformedData) {
            const response = await fetch('https://con-sl-rest.betha.cloud/contabil/service-layer/v2/api/devolucoes-receitas', {
                method: 'POST',
                headers: {
