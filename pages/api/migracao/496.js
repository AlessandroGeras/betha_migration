...
async function main() {
    try {
        ...
        const userQuery = `WITH UltimosBic AS (
    SELECT
        cd_Imovel,
...
RegistrosComNumero AS (
 ...
 when cd_InscricaoImovel = '0030040004' then 27629132
        ...
        398160 AS idAgrupamentos, -- código agrupamento GERAL
...
-- Primeira parte: nm_AreaEdificio
UNION ALL

select
ROW_NUMBER() OVER (ORDER BY IM.cd_Cecam) + 150000 as idIntegracao,
...
end as idImoveis, --id do imovel
                   398160 as idAgrupamentos, --código agrupamento GERAL
				   1262109 as idCamposAdicionais,
				   case when IM.nm_AnoConstrucao > 0 then nm_AnoConstrucao
				        else 1990
				   end as ano,
				   nm_AreaEdificio as vlCampo FOR JSON PATH, WITHOUT_ARRAY_WRAPPER)) AS imoveisInfComplem
from IPTUImoveis IM

-- Segunda parte: nm_AreaTerreno
UNION ALL

select
ROW_NUMBER() OVER (ORDER BY IM.cd_Cecam) + 200000 as idIntegracao,
JSON_QUERY((SELECT case
 when cd_InscricaoImovel = '0030040005' then 27629133
 when cd_InscricaoImovel = '0030040006' then 27629134
 when cd_InscricaoImovel = '0030040007' then 27629135
 ...
 when cd_InscricaoImovel = '0030040004' then 27629132
                   ...
                   398160 as idAgrupamentos, --código agrupamento GERAL
				   1262089 as idCamposAdicionais,
				   case when IM.nm_AnoConstrucao > 0 then nm_AnoConstrucao
				        else 1990
				   end as ano,
				   nm_areaterreno as vlCampo FOR JSON PATH, WITHOUT_ARRAY_WRAPPER)) AS imoveisInfComplem
from IPTUImoveis IM
`;
