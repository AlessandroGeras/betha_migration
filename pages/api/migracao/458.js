...
async function main() {
    try {
        ...
        const userQuery = `
            select
 ROW_NUMBER() OVER (ORDER BY s.nm_socio) AS idIntegracao,
 JSON_QUERY(
    (SELECT
100 as percSociedade,
case s.nm_socio 
...
when	'CONSELHO REGIONAL DE FISIOTERAPIA E TERAPIA OCUPACIONAL DA N'	then	77576020
when	'CAIXA ECONÔMICA FEDERAL'	then	77576040
END as idResponsavelLegal , 
CASE C.ds_razaosocial
when	'ALBERTO MARCELO C FACHINI'	then	77020440
when	'JOSE RODRIGUES COTRIM'	then	77020360
when	'GILBERTO FERREIRA GOMES'	then	77020420
when	'DENIVALDO FERNANDES DA SILVA'	then	77020380
...
) AS pessoasJuridicasSocios
from isssociodecl a
join ISSSocios s on s.cd_Socio = a.cd_Socio
join ISSContribuintes c on c.cd_Contribuinte = a.cd_Contribuinte
group by  s.nm_socio, C.ds_razaosocial
        `;
        ...
        const transformedData = resultData.map(record => {
            // Verificar se `pessoasJuridicasSocios` é uma string e parsear o JSON
            let content;
            try {
                content = typeof record.pessoasJuridicasSocios === 'string'
                    ? JSON.parse(record.pessoasJuridicasSocios)
                    : record.pessoasJuridicasSocios;
            } catch (error) {
                console.error('Erro ao fazer o parse de pessoasJuridicasSocios:', error);
                return null; // Ignorar o registro se houver erro no JSON
            }
            ...
            return {
                ...
                pessoasJuridicasSocios: {
                    percSociedade: content.percSociedade, // Usando o valor do percSociedade retornado
                    idPessoa: content.idPessoa, // Usando o valor do idPessoa retornado
                    idResponsavelLegal: content.idResponsavelLegal, // Usando o valor do idResponsavelLegal retornado
                    idSocio: content.idSocio // Usando o valor do idSocio retornado
                }
            };
