...
JSON_QUERY((SELECT case when cd_tipomovimento = 'AQ' then 11094
                                                when cd_tipomovimento = 'AJE' then 11092
                                                when cd_tipomovimento = 'DE' then 11107
...
ORDER BY al.cd_almoxa, al.aa_movimento, al.nr_sequencia ASC
        ...
        const transformedData = resultData.map(record => ({
            ...
            organograma: JSON.parse(record.organograma),
            naturezaMovimentacao: JSON.parse(record.naturezaMovimentacao),
            fornecedor: JSON.parse(record.fornecedor),
            dataCadastro: formatDate(record.dataCadastro),
            numeroComprovante: record.numeroComprovante,
        ...
        /* for (const record of transformedData) {
            const response = await fetch('https://almoxarifado.betha.cloud/estoque-services/api/conversoes/lotes/entradas', {
                method: 'POST',
                headers: {
