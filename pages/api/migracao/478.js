...
async function main() {
 ...
 when '122521' then 187785
                ...
                ) AS contaBancariaOrigem,
                'CORRENTE' AS tipoContaOrigem,
                JSON_QUERY(
                    (
                        SELECT
                            CASE concat(cb1.cd_codconta,cb1.cd_dig_conta)
 when '86428' then 187618
 ...
 when '040061' then 203099
                ...
                ) AS contaBancariaDestino,
                'CORRENTE' AS tipoContaDestino,
                m.cd_codigo_inicial AS numeroCadastro,
                'TRANSFERENCIA BANCARIA' AS finalidade,
                JSON_QUERY(
                    (
                        SELECT
                            m.dt_movto AS data,
                            JSON_QUERY(
                             ...
                             m.cd_codigo_inicial AS numeroDocumento
                        FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
                    )
                ) AS baixa
...
ORDER BY m.cd_codigo_inicial;
        ...
        const transformedData = resultData.map(record => {
            ...
            return {
                ...
                content: {
                    ...
                    valor: content.valor,
                    recurso: {
                        id: content.recurso && content.recurso.id ? content.recurso.id : null
                    },
                    contaBancariaOrigem: {
                        id: content.contaBancariaOrigem && content.contaBancariaOrigem.id ? content.contaBancariaOrigem.id : null
                    },
                    tipoContaOrigem: content.tipoContaOrigem,
                    contaBancariaDestino: {
                        id: content.contaBancariaDestino && content.contaBancariaDestino.id ? content.contaBancariaDestino.id : null
                    },
                    tipoContaDestino: content.tipoContaDestino,
                    numeroCadastro: content.numeroCadastro || null,
                    finalidade: content.finalidade,
                    baixa: content.baixa ? {
                        data: content.baixa.data ? formatDate(content.baixa.data) : null,
                        transacaoFinanceira: {
                            id: content.baixa.transacaoFinanceira && content.baixa.transacaoFinanceira.id ? content.baixa.transacaoFinanceira.id : null
                        },
                        numeroDocumento: content.baixa.numeroDocumento ? content.baixa.numeroDocumento.toString() : null
                    } : null
                }
            };
        ...
        for (const batch of batchedData) {
            try {
                ...
                const response = await fetch(`https://tes-sl-rest.betha.cloud/tesouraria/service-layer/v2/api/transferencias-bancarias`, {
                    method: 'POST',
                    headers: {
