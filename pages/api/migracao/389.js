...
where BP.cd_situacao = 1 and bp.dt_aquisicao < ('2024-01-01 00:00:00')-- and m.cd_tipomovto = '12'
...
const somaQuery = `
    SELECT 
        BP.nr_chapa, 
        SUM(vl_movimento) AS soma_valores
    FROM 
        PATRBensPatrimoniais BP
    JOIN 
        PAtrmovimentacao M ON M.nr_chapa = BP.nr_chapa
    WHERE 
        cd_tipomovto = 12 AND BP.cd_situacao = 1
    GROUP BY 
        BP.nr_chapa
`;

const resultsoma = await masterConnection.query(somaQuery);
const resultDatasoma = resultsoma.recordset;

const positiveValues = {}; // Inicialize o objeto para armazenar os valores positivos por nr_chapa

resultDatasoma.forEach((row) => {
    const nr_chapa = row['nr_chapa'];
    const value = row['soma_valores'];
    const numericValue = Number(value);
    if (!isNaN(numericValue)) {
        positiveValues[nr_chapa] = Math.abs(numericValue);
        //console.log(`nr_chapa: ${nr_chapa}, valor: ${value}, valor positivo: ${positiveValues[nr_chapa]}`);
    } else {
...
const transformedData = resultData.map((record) => {
    ...
    /* const nrChapa = record.nr_chapa; // Certifique-se de que este campo existe no resultado da primeira query
    const valorPositivo = positiveValues[nrChapa] || 0; */
    const somaValoresPositivo = Math.abs(resultDatasoma[contador].soma_valores);
    ...
    const transformedRecord = {
        conteudo: {
            ...
            bemValor: {
                ...
                vlDepreciavel: somaValoresPositivo,
    ...
    // Incrementar o contador e reiniciar se atingir o tamanho de resultDatasoma
    contador++;   
