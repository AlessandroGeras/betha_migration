...
CONCAT('INGRESSO EXTRAORÇAMENTARIO DA RECEITA: ', r.ds_receita)        as especificacao,
JSON_QUERY(
                        (SELECT
                        case  b.cd_codconta
...
where ct.cd_cecam = 3068 
and r.cd_ficharec > 5500 and r.cd_ficharec   <5600
...
const transformedData = resultData.map(record => {
    ...
    return {
        ...
        content: {
            exercicio: content.exercicio, // Coletando o valor de exercicio
            data: formatDate(content.data), // Formatando a data no formato AAAA-MM-DD
            transacaoFinanceira: {
                id: content.transacaoFinanceira?.id // Coletando o id da transacaoFinanceira
            },
            numeroDocumento: content.numeroDocumento.toString(), // Convertendo o número do documento para string
            valor: content.valor, // Coletando o valor
            especificacao: content.especificacao, // Coletando a especificação
            contaBancaria: {
                id: content.contaBancaria?.id // Coletando o id da conta bancária
            },
            tipoConta: content.tipoConta, // Coletando o tipo da conta (ex: CORRENTE)
            receitas: [
                {
                    receita: {
                        id: content.receitas?.receita?.id // Coletando o id da receita
                    },
                    valor: content.receitas?.valor, // Coletando o valor da receita
                    recursos: [
                        {
                            recurso: {
                                id: content.receitas?.recursos?.id // Coletando o id do recurso
                            },
                            valor: content.receitas?.recursos?.valor // Coletando o valor do recurso
                        }
                    ]
                }
            ]
        ...
        /* for (const record of transformedData) {
            const response = await fetch('https://con-sl-rest.betha.cloud/contabil/service-layer/v2/api/arrecadacoes-extras', {
                method: 'POST',
                headers: {
