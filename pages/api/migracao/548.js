...
where t.cd_AlteracaoOrcamentariaTipo in (3)
        ...
        const transformedData = resultData.map(record => {
            ...
            return {
                ...
                content: {
                    ...
                    // Conversão do campo "origens" para um array de objetos
                    origens: Array.isArray(content.origens)
                        ? content.origens.map(origem => ({
                            tipo: origem.tipo,
                            origem: {
                                id: origem.origem && origem.origem.id ? origem.origem.id : null
                            },
                            valor: origem.valor ? parseFloat(origem.valor) : null,
                            recursos: Array.isArray(origem.recursos) ? origem.recursos.map(recurso => ({
                                recurso: {
                        ...
                        ? [{
                            ...
                            recursos: Array.isArray(content.origens.recursos) ? content.origens.recursos.map(recurso => ({
                                recurso: {
                                    id: recurso.recurso && recurso.recurso.id ? recurso.recurso.id : null
                                },
                                valor: recurso.valor ? parseFloat(recurso.valor) : null
                            })) : content.origens.recursos
                                ? [{
                                    recurso: {
                    ...
                    // Conversão do campo "recursos" no nível principal
                    recursos: Array.isArray(content.recursos) ? content.recursos.map(recurso => ({
                        recurso: {
                            id: recurso.recurso && recurso.recurso.id ? recurso.recurso.id : null
                        },
                        valor: recurso.valor ? parseFloat(recurso.valor) : null
