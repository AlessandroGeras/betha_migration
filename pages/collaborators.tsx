...
const Collaborators = () => {
  ...
  const PDFPage = () => {
    useEffect(() => {
      ...
      // Define a posição inicial para adicionar o conteúdo da tabela
      let yPos = 10;

      // Itera sobre os documentos e adiciona cada linha da tabela ao PDF
      documents.docs.rows.forEach((document, index) => {
        const rowData = Object.keys(document).map((column) => document[column]).join(" | ");
        doc.text(rowData, 10, yPos);
        yPos += 10; // Ajusta a posição Y para a próxima linha
      });
