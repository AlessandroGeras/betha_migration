        ...
        {documents.success && (
          ...
          {/* LARGURA DA LISTA PRINCIPAL Fixa 1440px*/}
            ...
            {/* LARGURA DOS CAMPOS DE FILTRO INTERNOS */} 
              {filterOpen && (
                <div className={`flex text-gray-500 w-[2000px]`}>
            ...
            {/* LARGURA DO CONTEÚDO INTERNO DA LISTA PRINCIPAL */}
              {documents.docs.rows.map((document, index) => (
                <div className='w-[2000px]' key={document.id || Math.random().toString()}>
                  <div
                    className={`flex text-gray-700 whitespace-nowrap w-[2000px] text-ellipsis overflow-hidden  ${index % 2 === 0 ? 'bg-gray-100' : 'bg-gray-200'}`}
                  >
                    {Object.keys(columnWidths).map((column) => (
                      <div
                        key={column}
